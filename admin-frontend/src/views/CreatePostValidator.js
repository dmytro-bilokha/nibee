const fieldsValidationData =
  { title:
      { quickCheck: (value) =>
        /^[\x20-\x7F]*$/.test(value) ? undefined : 'Must contain only ASCII characters'
        , fullCheck: (value) =>
        /^[\x20-\x7F]{5,}$/.test(value) ? undefined
          : 'Must contain only ASCII characters and be at least 5 characters long'
      }
    , webPath:
      { quickCheck: (value) =>
        /^[a-zA-Z0-9-]*$/.test(value) ? undefined : 'Must contain only numbers, latin letters and minus sign'
        , fullCheck: (value) =>
